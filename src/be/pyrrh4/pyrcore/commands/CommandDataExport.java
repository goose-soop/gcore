...
public class CommandDataExport extends CommandArgument {
	...
	public CommandDataExport() {
		super(PyrCore.inst(), Utils.asList("export"), "generate a debug file (to send to the developper)", PCPerm.PYRCORE_ADMIN, false);
	}

	@Override
	public void perform(CommandCall call) {
		// temp root
		File tempRoot = new File(PyrCore.inst().getDataFolder() + "/data/temp/");
		if (tempRoot.exists()) {
			tempRoot.delete();
		}
		tempRoot.mkdirs();
		for (Plugin plugin : Bukkit.getPluginManager().getPlugins()) {
			if (plugin instanceof PyrPlugin) {
				try {
					FileUtils.copyDirectory(plugin.getDataFolder(), new File(tempRoot + "/" + plugin.getDataFolder().getName() + "/"));
				} catch (Throwable exception) {
					exception.printStackTrace();
					PyrCore.inst().error("Couldn't copy data folder of plugin " + plugin.getName());
				}
			}
		}
		// debug root
		...
		for (File logFile : Utils.asReverseList(Utils.asSortedList(Utils.asList(logRoot.listFiles())))) {
			...
			try {
				...
				exception.printStackTrace();
				Messenger.send(call.getSender(), Messenger.Level.SEVERE_ERROR, "PyrCore", "Couldn't move latest log file to the debug folder.");
			}
		}
		...
		try {
			...
			exception.printStackTrace();
			Messenger.send(call.getSender(), Messenger.Level.SEVERE_ERROR, "PyrCore", "Couldn't create server info file.");
		}
		// zip
		File target = new File(new File(".").getAbsolutePath() + File.separator + "plugins" + File.separator + "PyrCore_" + new SimpleDateFormat("YYYY'-'MM'-'dd'_'HH'-'mm").format(Calendar.getInstance().getTime()) + ".zip");
		...
		// message
		Messenger.send(call.getSender(), Messenger.Level.NORMAL_SUCCESS, "PyrCore", "Debug file exported to <" + target.getPath() + ">. Send this zip file to the developper !");
	}
