...
import be.pyrrh4.pyrcore.libs.org.bstats.Metrics;

public abstract class PyrPlugin extends JavaPlugin implements Listener {
			...
			if (!secondary) {
				...
				if (command.getChild("reload") == null) {
					command.addChild(new CommandArgument(this, Utils.asList("reload", "rl"), "reload the plugin", adminPermission, false) {
						...
						protected void perform(CommandCall call) {
							...
							reload();
							PCLocale.MSG_GENERIC_RELOAD.send(call.getSender(), "{plugin}", getName(), "{time}", (System.currentTimeMillis() - start) + "ms");
						}
					});
				}
				...
				if (command.getChild("support") == null) {
					command.addChild(new CommandArgument(this, Utils.asList("support", "bug", "bugreport", "report", "suggest", "discord"), "get support", adminPermission, false) {
						@Override
						protected void perform(CommandCall call) {
							Messenger.send(call.getSender(), Messenger.Level.NORMAL_INFO, "PyrCore", "You can get help, report bugs and suggest features on my discord server ! :D");
							Messenger.send(call.getSender(), Messenger.Level.NORMAL_INFO, "PyrCore", "http://www.pyrrh4.be/plugins/discord/");
						}
					});
				}
			}
	...
	public void onEnable() {
		...
		if (PyrCore.inst().errorEnable()) {
			cancelEnable("An error occured while enabling PyrCore");
			return;
		}
