...
public class CommandArgument {
	...
	public CommandArgument(PyrPlugin plugin, List<String> aliases, String description, Perm permission, boolean playerOnly, Param... expectedParams) {
		this(plugin, null, aliases, description, permission, playerOnly, expectedParams);
	}

	public CommandArgument(PyrPlugin plugin, CommandArgument parent, List<String> aliases, String description, Perm permission, boolean playerOnly, Param... expectedParams) {
		this.plugin = plugin;
		this.parent = parent;
	...
	public boolean call(CommandCall call, int argumentIndexForThis) {
		...
		if (argumentIndexForThis + 1 < call.getArguments().size()) {
			// no children
			if (children.isEmpty()) {
				PCLocale.MSG_GENERIC_COMMAND_NOCHILDREN.send(call.getSender(), "{plugin}", plugin.getName(), "{current_path}", getFullName());
				return true;
			}
			...
			PCLocale.MSG_GENERIC_COMMAND_NOCHILDPERFORMED.send(call.getSender(), "{plugin}", plugin.getName(), "{current_path}", getFullName());
			return true;
		}
		// sender isn't a player, so return false
		if (playerOnly ? !call.senderIsPlayer() : false) {
			PCLocale.MSG_GENERIC_NOTPLAYER.send(call.getSender(), "{plugin}", plugin.getName());
			return true;
		}
		// sender doesn't have permission, so return false
		if (permission == null ? false : !permission.has(call.getSender())) {
			PCLocale.MSG_GENERIC_NOPERMISSION.send(call.getSender(), "{plugin}", plugin.getName());
			return true;
		}
