...
public class Param {
	...
	public boolean ensureAuthorization(CommandCall call) {
		...
		if (playerOnly ? !call.senderIsPlayer() : false) {
			PCLocale.MSG_GENERIC_NOTPLAYER.send(call.getSender(), "{plugin}", call.getPlugin().getName());
			return false;
		}
		// sender doesn't have permission, so return false
		if (permission == null ? false : !permission.has(call.getSender())) {
			PCLocale.MSG_GENERIC_NOPERMISSION.send(call.getSender(), "{plugin}", call.getPlugin().getName());
			return false;
		}
		// good
		return true;
	...
	public String getString(CommandCall call) {
		...
		PCLocale.MSG_GENERIC_COMMAND_MISSINGPARAM.send(call.getSender(), "{plugin}", call.getPlugin().getName(), "{parameter}", "-" + toString() + (getDescription() == null ? "" : ":[" + getDescription() + "]"));
		return null;
	}
	...
	public String getStringAlphanumeric(CommandCall call) {
		...
		if (value != null) {
			...
			// not alphanumeric
				...
				PCLocale.MSG_GENERIC_COMMAND_INVALIDALPHANUMERICPARAM.send(call.getSender(), "{plugin}", call.getPlugin().getName(), "{parameter}", "-" + toString(), "{value}", value);
			}
		}
		return null;
	}
	...
	public UUID getUUID(CommandCall call) {
		...
		if (value != null) {
			...
			// not a number
				...
				PCLocale.MSG_GENERIC_COMMAND_INVALIDUUIDPARAM.send(call.getSender(), "{plugin}", call.getPlugin().getName(), "{parameter}", "-" + toString(), "{value}", value);
			}
		}
		return null;
	}
	...
	public int getInt(CommandCall call) {
		...
		if (value != null) {
			...
			// not a number
				...
				PCLocale.MSG_GENERIC_COMMAND_INVALIDINTPARAM.send(call.getSender(), "{plugin}", call.getPlugin().getName(), "{parameter}", "-" + toString(), "{value}", value);
			}
		}
	...
	public double getDouble(CommandCall call) {
		...
		if (value != null) {
			...
			// not a number
				...
				PCLocale.MSG_GENERIC_COMMAND_INVALIDDOUBLEPARAM.send(call.getSender(), "{plugin}", call.getPlugin().getName(), "{parameter}", "-" + toString(), "{value}", value);
			}
		}
		return Integer.MIN_VALUE;
	}
	...
	public OfflinePlayer getOfflinePlayer(CommandCall call, boolean senderIfNone) {
		...
		// not a player
			...
			PCLocale.MSG_GENERIC_COMMAND_INVALIDOFFLINEPLAYERPARAM.send(call.getSender(), "{plugin}", call.getPlugin().getName(), "{parameter}", "-" + toString(), "{value}", value);
			return null;
		}
	}
	...
	public <T extends Enum<T>> T getEnum(CommandCall call, Class<T> enumClass) {
		...
		if (value != null) {
			...
			// unknown enum
			PCLocale.MSG_GENERIC_COMMAND_INVALIDENUMPARAM.send(call.getSender(), "{plugin}", call.getPlugin().getName(), "{parameter}", "-" + toString(), "{value}", value, "{enum}", enumClass.getSimpleName());
		}
		return null;
	...
	public Player getPlayer(CommandCall call, boolean senderIfNone) {
		...
		// not a player
			...
			PCLocale.MSG_GENERIC_COMMAND_INVALIDPLAYERPARAM.send(call.getSender(), "{plugin}", call.getPlugin().getName(), "{parameter}", "-" + toString(), "{value}", value);
			return null;
		}
	}
