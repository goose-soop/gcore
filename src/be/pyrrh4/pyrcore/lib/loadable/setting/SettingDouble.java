...
public class SettingDouble extends AbstractUniqueSetting<Double> {
	...
	// base
	public SettingDouble(String id, String def, boolean mandatory, List<String> description) {
		super(id, def, mandatory, "decimal number", description);
	}

	// methods
	@Override
	public Double parse(String raw) {
		return getParsed(raw);
	}

	public static Double getParsed(String raw) {
		try {
			return new Double(Math.floor(Utils.calculateExpression(raw)));
			...
			protected void onClick(Player player, ClickType clickType, int pageIndex) {
				...
				PyrCore.inst().getChatInputs().put(player, new ChatInput() {
					...
					public void onChat(Player player, String value) {
						// TODO : verify that value contains either a valid double, either a PAPI variable, either a QC variable
