...
public class SettingPotionEffectType extends AbstractUniqueSetting<PotionEffectType> {

	// base
	public SettingPotionEffectType(String id, String def, boolean mandatory, List<String> description) {
		super(id, def, mandatory, "potion effect type", description);
	}

	// get
	@Override
	public PotionEffectType parse(String raw) {
		return Utils.potionEffectTypeOrNull(raw);
	}
							...
							protected void onClick(Player player, ClickType clickType, int pageIndex) {
								...
								EditorGUI subEnum = new EditorGUI(subThis, "Select") {
									...
									protected void fill() {
										// values
										int valueSlot = -1;
										for (final PotionEffectType value : Utils.asSortedList(Utils.asList(PotionEffectType.values()), Utils.effectSorter)) {
