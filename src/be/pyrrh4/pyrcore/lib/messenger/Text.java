...
public class Text {
	...
	public Text() {
		this("unknown_" + UUID.randomUUID().toString().split("-")[0], PCLocale.fileUnknown);
	}

	public Text(Object... defaultLangs) {
		this("unknown_" + UUID.randomUUID().toString().split("-")[0], PCLocale.fileUnknown, defaultLangs);
	}
	...
	public Text(String id, File localeFile, Object... defaultLangs) {
		...
		if (defaultLangs != null) {
			if (defaultLangs.length % 2 != 0) {
				PyrCore.inst().error("Trying to register message " + id + " but default langs size is invalid");
			} else {
	...
	public List<String> getActiveLines() {
		return langs.containsKey(PyrCore.inst().getActiveLocaleLang()) ? langs.get(PyrCore.inst().getActiveLocaleLang()) : getDefaultLines();
	}
