...
import be.pyrrh4.pyrcore.lib.parseable.primitive.PPStringList;
import be.pyrrh4.pyrcore.lib.versioncompat.Compat;

public class CPItem extends ContainerParseable {

	// base
	private PPMat type = addComponent(new PPMat("type", this, null, true, 0, EditorGUI.ICON_ITEM, PCLocale.GUI_GENERIC_EDITOR_ITEM_TYPELORE.getLines()));
	private PPInteger durability = addComponent(new PPInteger("durability", this, "0", 0, Integer.MAX_VALUE, false, 1, EditorGUI.ICON_NUMBER, PCLocale.GUI_GENERIC_EDITOR_ITEM_DURABILITYLORE.getLines()));
	private PPBoolean unbreakable = addComponent(new PPBoolean("unbreakable", this, "false", false, 2, EditorGUI.ICON_BOOLEAN, PCLocale.GUI_GENERIC_EDITOR_ITEM_UNBREAKABLELORE.getLines()));
	private PPInteger amount = addComponent(new PPInteger("amount", this, "1", 1, Integer.MAX_VALUE, false, 3, EditorGUI.ICON_NUMBER, PCLocale.GUI_GENERIC_EDITOR_ITEM_AMOUNTLORE.getLines()));
	private PPInteger maxAmount = addComponent(new PPInteger("max_amount", this, "0", 0, Integer.MAX_VALUE, false, 4, EditorGUI.ICON_NUMBER, PCLocale.GUI_GENERIC_EDITOR_ITEM_MAXAMOUNTLORE.getLines()));
	private PPString name = addComponent(new PPString("name", this, null, false, 5, EditorGUI.ICON_STRING, PCLocale.GUI_GENERIC_EDITOR_ITEM_NAMELORE.getLines()));
	private PPStringList lore = addComponent(new PPStringList("lore", this, null, false, 6, EditorGUI.ICON_STRING_LIST, PCLocale.GUI_GENERIC_EDITOR_ITEM_LORELORE.getLines()));
	/* TODO : make this a LPEnchantment */ private PPStringList enchants = addComponent(new PPStringList("enchants", this, null, false, 7, EditorGUI.ICON_ENCHANTMENT, PCLocale.GUI_GENERIC_EDITOR_ITEM_ENCHANTSLORE.getLines()));
	private PPString nbt = addComponent(new PPString("nbt", this, null, false, 8, EditorGUI.ICON_NBT, PCLocale.GUI_GENERIC_EDITOR_ITEM_NBTLORE.getLines()));
	private PPBoolean enabled = addComponent(new PPBoolean("enabled", this, "true", false, 9, EditorGUI.ICON_BOOLEAN, PCLocale.GUI_GENERIC_EDITOR_ITEM_ENABLEDLORE.getLines()));
	private PPInteger slot = addComponent(new PPInteger("slot", this, "-1", -1, 54, false, 10, EditorGUI.ICON_NUMBER, PCLocale.GUI_GENERIC_EDITOR_ITEM_SLOTLORE.getLines()));
	private PPDouble chance = addComponent(new PPDouble("chance", this, "0", -1d, 100d, false, 11, EditorGUI.ICON_NUMBER, PCLocale.GUI_GENERIC_EDITOR_ITEM_CHANCELORE.getLines()));
	private PPBoolean mustHaveInHand = addComponent(new PPBoolean("must_have_in_hand", this, "false", false, 12, EditorGUI.ICON_BOOLEAN, PCLocale.GUI_GENERIC_EDITOR_ITEM_MUSTHAVEINHANDLORE.getLines()));
	private PPBoolean removeAfterAction = addComponent(new PPBoolean("remove_after_action", this, "false", false, 13, EditorGUI.ICON_BOOLEAN, PCLocale.GUI_GENERIC_EDITOR_ITEM_REMOVEAFTERACTIONLORE.getLines()));
		...
		if ((parser != null && !cache.containsKey(parser.getUniqueId())) || !cache.containsKey(null)) {
			...
			if (nbt != null) {
				try {
					...
					PyrCore.inst().error("Couldn't load NBT from " + nbt + " for item " + getId());
				}
			}
