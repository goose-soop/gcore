package be.pyrrh4.pyrcore.libs.org.bstats;
...
public class Metrics {
	...
	static {
		...
		final String examplePackage = new String(new byte[] { 'b', 'e', '.', 'p', 'y', 'r', 'r', 'h', '4' });
	...
	// The plugin
	private final PyrPlugin plugin;
	 ...
	 */
	public Metrics(PyrPlugin plugin) {
		if (plugin == null) {
			...
			public void run() {
				try {
					...
					if (logFailedRequests) {
						e.printStackTrace();
						Logger.log(Level.WARNING, plugin, "Could not submit plugin stats");
					}
				}
			}
		}).start();
	...
	public static abstract class CustomChart {
		...
		protected JSONObject getRequestJsonObject() {
			...
			try {
				...
				if (logFailedRequests) {
					t.printStackTrace();
					Logger.log(Level.INFO, PyrCore.inst(), "[Metrics] Failed to get data for custom chart with id " + chartId);
				}
				return null;
			}
			return chart;
