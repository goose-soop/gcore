...
public final class ItemUtils {
	...
	public static String describeSingleLine(ItemStack item) {
		return StringUtils.toTextString(" ", describe(item, TextGeneric.textDescribeItemSingleLine));
	}

	public static List<String> describe(ItemStack item) {
		return describe(item, TextGeneric.textDescribeItem);
	}

	public static List<String> describe(ItemStack item, Text text) {
		...
		return text
				...
				.replace("{custom_model_data}", () -> Version.ATLEAST_1_14 ? item.getItemMeta().getCustomModelData() : null)
				.replace("{name}", () -> meta == null || !meta.hasDisplayName() ? "" : meta.getDisplayName())
				.replace("{lore}", () -> {
					...
					if (lore == null || lore.isEmpty()) {
						...
						return TextGeneric.textDescribeItemLoreIfHas.replace("{lore}", () -> parsed).parseLines();
					}
				})
				.replace("{enchantments}", () -> {
					if (item.getEnchantments().isEmpty()) {
						...
						List<String> parsed = new ArrayList<>();
						item.getEnchantments().forEach((enchant, level) -> parsed.addAll(TextGeneric.textDescribeItemEnchantmentsLine.replace("{enchant}", () -> enchant.getName()).replace("{level}", () -> level).parseLines()));
						return TextGeneric.textDescribeItemEnchantmentsIfHas.replace("{enchantments}", () -> parsed).parseLines();
					}
				})
				.parseLines();
	}
	...
	public static int take(Player player, ItemStack item, ItemCheck check, boolean updateInv) {
		return take(player, item, item.getAmount(), check, updateInv);
	}
	...
	public static boolean has(Player player, ItemStack item, ItemCheck check) {
		return has(player, item, item.getAmount(), check);
	}
	...
	// similarity
	private static final Map<Enchantment, Integer> EMPTY_ENCHANTS = new HashMap<>();
	private static final List<ItemFlag> EMPTY_FLAGS = new ArrayList<>();
	...
	public static boolean match(ItemStack item, ItemStack reference, ItemCheck check) {
		...
		boolean voidReference = Mat.isVoid(reference);
		...
		if (reference.hasItemMeta()) {
			// - don't check if itemMeta is null but referenceMeta isn't ; sometimes the meta exists but is empty
			...
			boolean referenceUnbreakable = Compat.isUnbreakable(referenceMeta);
			...
			if (check.isExact()) {
				...
				if (itemHasEnchants != referenceMeta.hasEnchants()) return false;
				Map<Enchantment, Integer> itemEnchants = itemMeta == null ? EMPTY_ENCHANTS : itemMeta.getEnchants();
				if (!CollectionUtils.contentEquals(itemEnchants, referenceMeta.getEnchants())) return false;
			} else {
				if (referenceMeta.hasEnchants() && (itemMeta == null || !itemMeta.hasEnchants())) return false;
				for (Enchantment enchantment : referenceMeta.getEnchants().keySet()) {
					if (itemMeta.getEnchants().get(enchantment) != referenceMeta.getEnchants().get(enchantment)) {
						return false;
					}
				}
			}
			...
			List<ItemFlag> referenceFlags = referenceMeta == null ? EMPTY_FLAGS : Compat.getItemFlags(referenceMeta);
			...
			// name
			ConfigGCore.logspamItemNbt(null, () -> "Item match ; name");
			String itemName = itemMeta == null ? null : (!itemMeta.hasDisplayName() ? null : itemMeta.getDisplayName());
			String referenceName = !referenceMeta.hasDisplayName() ? null : referenceMeta.getDisplayName();
			if (check.isExact()) {
				if (!Objects.deepEquals(itemName, referenceName)) {
					return false;
				}
			} else {
				if (check.nameContains()) {
					...
					if (referenceName != null && (itemName == null || !itemName.equals(referenceName))) {
						return false;
					}
				}
			}
			...
			List<String> referenceLore = !referenceMeta.hasLore() ? EMPTY_LORE : referenceMeta.getLore();
			if (check.isExact()) {
			...
			if (!MetaBook.match(itemMeta, referenceMeta, check)) return false;
			ConfigGCore.logspamItemNbt(null, () -> "Item match ; enchantment storage meta");
			if (!MetaEnchantmentStorage.match(itemMeta, referenceMeta, check)) return false;
			ConfigGCore.logspamItemNbt(null, () -> "Item match ; firework effect meta");
			if (!MetaFireworkEffect.match(itemMeta, referenceMeta, check)) return false;
			ConfigGCore.logspamItemNbt(null, () -> "Item match ; firework meta");
			if (!MetaFirework.match(itemMeta, referenceMeta, check)) return false;
			ConfigGCore.logspamItemNbt(null, () -> "Item match ; leather armor meta");
			if (!MetaLeatherArmor.match(itemMeta, referenceMeta, check)) return false;
			ConfigGCore.logspamItemNbt(null, () -> "Item match ; potion meta");
			if (!MetaPotion.match(itemMeta, referenceMeta, check)) return false;
			ConfigGCore.logspamItemNbt(null, () -> "Item match ; skull meta");
			if (!MetaSkull.match(itemMeta, referenceMeta, check)) return false;
			ConfigGCore.logspamItemNbt(null, () -> "Item match ; banner meta");
			if (Version.ATLEAST_1_8 && !com.guillaumevdn.gcore.lib.item.meta.MetaBanner.match(itemMeta, referenceMeta, check)) return false;
			ConfigGCore.logspamItemNbt(null, () -> "Item match ; spawn egg meta");
			if (Version.ATLEAST_1_11 && !Version.ATLEAST_1_13 && !com.guillaumevdn.gcore.lib.item.meta.MetaSpawnEgg.match(itemMeta, referenceMeta, check)) return false;
			ConfigGCore.logspamItemNbt(null, () -> "Item match ; knowledge book meta");
			if (Version.ATLEAST_1_12 && !com.guillaumevdn.gcore.lib.item.meta.MetaKnowledgeBook.match(itemMeta, referenceMeta, check)) return false;
			ConfigGCore.logspamItemNbt(null, () -> "Item match ; tropical fish bucket meta");
			if (Version.ATLEAST_1_13 && !com.guillaumevdn.gcore.lib.item.meta.MetaTropicalFishBucket.match(itemMeta, referenceMeta, check)) return false;
			ConfigGCore.logspamItemNbt(null, () -> "Item match ; crossbow meta");
			if (Version.ATLEAST_1_14 && !com.guillaumevdn.gcore.lib.item.meta.MetaCrossbow.match(itemMeta, referenceMeta, check)) return false;
			ConfigGCore.logspamItemNbt(null, () -> "Item match ; suspicious stew meta");
			if (Version.ATLEAST_1_15 && !com.guillaumevdn.gcore.lib.item.meta.MetaSuspiciousStew.match(itemMeta, referenceMeta, check)) return false;
		}
		// nbt
		try {
			ConfigGCore.logspamItemNbt(null, () -> "Item match ; nbt");
			NBTItem nbt = new NBTItem(item);
			NBTItem nbtRef = new NBTItem(reference);
