...
public abstract class Pathfinding {
	...
	private List<List<Point>> sortedPaths = null;

	// constructor
	public Pathfinding(World world, Point start, Point target, int step, int speed, int yToleranceUp, int yToleranceDown, double targetDistanceTolerance) {
		this.world = world;
		this.start = start;
		this.target = target;
		this.step = step;
		...
		this.relativesOffsets = CollectionUtils.asList( // horizontal movements
				Pair.of(0, step),
				Pair.of(0, -step),
				Pair.of(step, 0),
				Pair.of(-step, 0)
	...
	public void start() {
		...
		toExplore = CollectionUtils.asSet(start);
		available = new HashSet<>();
		distances = CollectionUtils.asMap(start, start.distance(target));
		...
		task = new BukkitRunnable() {
			...
			public void run() {
				...
				if (state.equals(State.OPTIMIZING)) {
					// sort paths
					sortedPaths = CollectionUtils.asList(paths.keySet());
					sortedPaths.sort((o1, o2) -> Double.compare(paths.get(o1), paths.get(o2)));
					// find shortest path
					List<Point> shortest = null;
					double targetTolerance = targetDistanceTolerance;
		...
		}.runTaskTimerAsynchronously(GCore.inst(), 0L, 1L);
	}
		...
		while ((explore = getNextExplorationPoint()) != null && ++limit <= speed) {
			...
			if (!alreadyHasOnePath) {
				...
				for (Pair<Integer, Integer> offset : relativesOffsets) {
					...
					// get shortest
					List<Double> sorted = CollectionUtils.asSortedList(containing.values(), Double::compare);
				...
				// add or create path
				if (closestPath != null) {
					List<Point> newPath = CollectionUtils.asListMultiple(Point.class, closestPath.getA(), explore);
					...
					paths.put(CollectionUtils.asList(start, explore), start.distance(explore));
				}
			}
			// get points around
			for (Pair<Integer, Integer> offset : relativesOffsets) {
	...
	private List<Point> getShortestPath(double targetDistanceTolerance) {
		List<Point> shortest = null;
		for (List<Point> path : sortedPaths) {
			boolean valid = false;
			for (Point point : path) {
				if (point.distance(target) <= targetDistanceTolerance) {
					valid = true;
					break;
				}
			}
	...
	private Point getNextExplorationPoint() {
		...
		// sort distances
		List<Double> sorted = CollectionUtils.asSortedList(list, Double::compare);
		double first = sorted.get(0);
		for (Point explore : toExplore) {
	...
	private Point getAvailableForMoving(Point point) {
		// not free
		if (!Mat.fromBlock(world.getBlockAt(point.getX(), point.getY(), point.getZ())).get().getData().isTraversable()) {
			...
			Mat bmat = Mat.fromBlock(world.getBlockAt(available.getX(), available.getY() - 1, available.getZ())).get();
			if (bmat.toString().contains("FENCE") || bmat.toString().contains("WALL")) {
				return null;
			}
			// not enough space above the target block to pass or jump up
			if (!Mat.fromBlock(world.getBlockAt(available.getX(), available.getY() + 1, available.getZ())).get().getData().isTraversable() || !Mat.fromBlock(world.getBlockAt(available.getX(), available.getY() + 2, available.getZ())).get().getData().isTraversable()) {
				return null;
			}
			// good for jumping up
			return available;
		...
		// free
			...
			if (!Mat.fromBlock(world.getBlockAt(point.getX(), point.getY() + 1, point.getZ())).get().getData().isTraversable()) {
				return null;
			}
	...
	private Point getLastTraversableDown(Point point) {
		...
		while (y-- > 0) {// -- so we check the argument on first iteration
			...
			if (Mat.fromBlock(block).get().getData().isTraversable()) {
