...
/**
 * Represents the configuration and data of a behavior event
 */
public abstract class BEvent extends ContainerParseable {

	// base
	private BEventType type;
	private PPString process = addComponent(new PPString("process", this, null, false, 1, EditorGUI.ICON_STRING, GLocale.GUI_GCORE_EDITOR_NPC_BEHAVIOREVENTPROCESSLORE.getLines()));

	public BEvent(String id, BEventType type, Parseable parent, boolean mandatory, int editorSlot, Mat editorIcon, List<String> editorDescription) {
		super(id, parent, "npc behavior event", mandatory, editorSlot, editorIcon, editorDescription);
		this.type = type;
	}

	// get
	public BEventType getType() {
		return type;
	}

	public PPString getProcess() {
		return process;
	}

	public String getProcess(Player parser) {
		return process.getParsedValue(parser);
	}
	 ...
	 * @return true if the event was triggered and must run its process
	 */
	public abstract boolean triggerAttempt(AttemptContext context, Player player, Npc npc, Event event);

	public static enum AttemptContext {
		EVENT, TIMER, CALL;
	}
	...
	protected void fillEditor(final EditorGUI gui, Player player, final ModifCallback onModif) {
		// add type selector
		gui.setRegularItem(new EditorItem("object_type", 0, Mat.ENDER_CHEST, "§6type", GLocale.GUI_GCORE_EDITOR_NPC_BEHAVIOREVENTTYPELORE.getLines()) {
			@Override
						...
						setRegularItem(new EditorItem("control_item_select", 2, Mat.ENDER_CHEST, GLocale.GUI_GENERIC_EDITORENUMSELECT.getLine(), GLocale.GUI_GENERIC_EDITORENUMSELECTLORE.getLines()) {
							...
							protected void onClick(final Player player, final ClickType clickType, final int pageIndex) {
								...
								EditorGUI subSelection = new EditorGUI(getLastData().getPlugin(), gui, Utils.getNewInventoryName(gui.getName(), "Select"), 54, GUI.SLOTS_0_TO_44) {
									...
									protected void fill() {
										// add values
										for (final BEventType val : Utils.asSortedList(BEventType.values(), Utils.objectSorter)) {
											final String valName = val.getId();
											setRegularItem(new EditorItem("value_" + valName, -1, Mat.ENDER_CHEST, "§6" + valName, null) {
												@Override
												protected void onClick(final Player player, final ClickType clickType, final int pageIndex) {
													BEvent curr = BEvent.this;
													final BEvent result = val.createNew(curr.getId(), curr.getParent(), curr.getLastData().clone(), false, curr.isMandatory(), curr.getEditorSlot(), curr.getEditorIcon(), curr.getEditorDescription());
													// error loading
													if (result == null) {
													...
													// success
														...
														// replace result
														try {
															((ParseableContainment<BEvent>) BEvent.this.getParent()).replaceContaining(result);
	 ...
	 * @return the loaded behavior event, or null if couldn't instantiate it for some reason
	 */
	public static BEvent load(String id, Parseable parent, ConfigData data, boolean mandatory, int editorSlot, Mat editorIcon, List<String> editorDescription) {
		// compact
		BEventType type = null;
		// missing type setting
		data.setContains(data.getConfig().contains(data.getPath() + ".type"));
		if (!data.contains()) {
			data.log("missing primitive setting 'type' (must be a behavior event type)");
			return null;
		}
		// invalid type
		type = BEventType.valueOf(data.getConfig().getString(data.getPath() + ".type", ""));
		if (type == null) {
			data.log("invalid primitive setting 'type' (must be a behavior event type)");
			return null;
		}
		// create
