...
public final class ListenerDeluxeChatJSONEvent {
	...
	public static final void register(IntegrationDeluxeChat instance) {
		...
		/*
		 * instance.registerEvent(DeluxeChatJSONEvent.class, EventPriority.LOWEST, true, og -> { if (og.getPlayer() != null) {
		 * PlayerChatEvent event = PlayerChatEvent.call(og.getPlayer(), og.getJSONChatMessage(),
		 * CollectionUtils.asSet(og.getPlayer())); og.setJSONChatMessage(event.getMessage());
