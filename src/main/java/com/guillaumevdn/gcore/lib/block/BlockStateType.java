...
public enum BlockStateType implements LinearObjectType {
	...
	ATTACHED(CommonMats.WHITE_WOOL, "Attachable", (state, data) -> data.invokeMethod("isAttached").get(boolean.class) == state.getValidBooleanArgument(0),
	...
	FACE_ATTACHABLE(CommonMats.LEVER, "FaceAttachable",
			(state, data) -> data.invokeMethod("getAttachedFace").get()
					.equals(state.getValidUnknownEnumTypeArgument("org.bukkit.block.data.FaceAttachable$AttachedFace", 0)),
			(state, data) -> data.invokeMethod("setAttachedFace",
					state.getValidUnknownEnumTypeArgument("org.bukkit.block.data.FaceAttachable$AttachedFace", 0))),
	FACING(CommonMats.RED_BED, "Directional", (state, data) -> data.invokeMethod("getFacing").get().equals(state.getValidEnumTypeArgument(BlockFace.class, 0)),
			(state, data) -> data.invokeMethod("setFacing", state.getValidEnumTypeArgument(BlockFace.class, 0))),
	FACING_MULTIPLE(CommonMats.RED_BED, "MultipleFacing", (state, data) -> {
	...
	LIT(CommonMats.TORCH, "Lightable", (state, data) -> data.invokeMethod("isLit").get(boolean.class) == state.getValidBooleanArgument(0),
			(state, data) -> data.invokeMethod("setLit", state.getValidBooleanArgument(0))),
	HONEY_LEVEL(CommonMats.TORCH, "type.Beehive", (state, data) -> data.invokeMethod("getHoneyLevel").get(int.class) == state.getValidIntArgument(0),
			(state, data) -> data.invokeMethod("setHoneyLevel", state.getValidIntArgument(0))),
	OPENED(CommonMats.IRON_DOOR, "Openable", (state, data) -> data.invokeMethod("isOpen").get(boolean.class) == state.getValidBooleanArgument(0),
			(state, data) -> data.invokeMethod("setOpen", state.getValidBooleanArgument(0))),
	POWER(CommonMats.REDSTONE, "AnaloguePowerable", (state, data) -> data.invokeMethod("getPower").get(int.class) == state.getValidIntArgument(0),
			(state, data) -> data.invokeMethod("setPower", state.getValidIntArgument(0))),
	POWERED(CommonMats.REDSTONE, "Powerable", (state, data) -> data.invokeMethod("isPowered").get(boolean.class) == state.getValidBooleanArgument(0),
	...
	ROTATION(CommonMats.RAIL, "Rotatable", (state, data) -> data.invokeMethod("getRotation").get().equals(state.getValidEnumTypeArgument(BlockFace.class, 0)),
			(state, data) -> data.invokeMethod("setRotation", state.getValidEnumTypeArgument(BlockFace.class, 0))),
	SNOWY(CommonMats.ICE, "Snowy", (state, data) -> data.invokeMethod("isSnowy").get(boolean.class) == state.getValidBooleanArgument(0),
			(state, data) -> data.invokeMethod("setSnowy", state.getValidBooleanArgument(0))),
	LEVEL(CommonMats.WATER_BUCKET, "Levelled", (state, data) -> data.invokeMethod("getLevel").get(int.class) == state.getValidIntArgument(0),
	...
	BlockStateType(Mat icon, String dataClass, ThrowableBiFunction<BlockState, ReflectionObject, Boolean> checker,
			ThrowableBiConsumer<BlockState, ReflectionObject> applier) {
		this.icon = icon;
		try {
	...
	public boolean has(BlockState blockState, org.bukkit.block.BlockState block) {
		// not supported
		if (dataClass == null) {
			GCore.inst().getMainLogger().error("Can't check block state '" + name() + " " + StringUtils.toTextString(" ", blockState.getArguments())
					+ "', it's not supported on version " + Version.CURRENT);
			return false;
		}
		try {
			// invalid state
			...
			GCore.inst().getMainLogger().error("Can't check block state '" + name() + " " + StringUtils.toTextString(" ", blockState.getArguments())
					+ "' for block " + Mat.fromBlock(block) + " on version " + Version.CURRENT, exception);
			return false;
		}
	}
	...
	public void set(BlockState blockState, Block block) {
		// not supported
		if (dataClass == null) {
			GCore.inst().getMainLogger().error("Can't set block state '" + name() + " " + StringUtils.toTextString(" ", blockState.getArguments())
					+ "', it's not supported on version " + Version.CURRENT);
			return;
		}
		try {
			...
			if (data == null || !ObjectUtils.instanceOf(data.justGet(), dataClass)) {
				GCore.inst().getMainLogger().error("Can't set block state '" + name() + " " + StringUtils.toTextString(" ", blockState.getArguments())
						+ "', block " + Mat.fromBlock(block) + " doesn't match");
				return;
			}
			...
			GCore.inst().getMainLogger().error("Can't set block state '" + name() + " " + StringUtils.toTextString(" ", blockState.getArguments())
					+ "' to block " + Mat.fromBlock(block) + " on version " + Version.CURRENT, exception);
		}
	}
