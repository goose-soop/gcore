...
public final class GenericPluginState extends Subcommand {
	...
	public GenericPluginState(GPlugin<?, ?> plugin) {
		super(false, plugin.getPermissionContainer().getAdminPermission(), TextGeneric.commandDescriptionGenericPluginState,
				ConfigGCore.genericCommandsAliasesPluginState);
		this.plugin = plugin;
	}

	@Override
	public void perform(CommandCall call) {
		TextGCore.messagePluginInternalState.replace("{plugin}", () -> plugin.getName()).replace("{version}", () -> plugin.getDescription().getVersion())
				.replace("{lifecycle_reference}", () -> "" + plugin.getLifecycleReference().hashCode()).replace("{data_boards}", () -> {
					...
					List<String> desc = CollectionUtils.asList("");
					plugin.getGUIs()
							.forEach(value -> desc.add(
									"§a" + value.getId() + " §7= §2" + value.getType() + ", " + StringUtils.pluralizeAmountDesc("page", value.getPageCount())
											+ ", " + StringUtils.pluralizeAmountDesc("viewer", value.getViewers().size())));
					desc.sort(String::compareTo);
					return desc;
				}).replace("{tasks}", () -> {
					...
					List<String> desc = CollectionUtils.asList("");
					plugin.getIntegrations()
							.forEach((id, value) -> desc.add(
									"§a" + value.getPluginName() + " §7= §2" + (!PluginUtils.isPluginEnabled(value.getPluginName()) ? "§eplugin not installed"
											: (value.isActivated() ? "§aenabled" : "§cdisabled"))));
					desc.sort(String::compareTo);
					return desc;
				}).replace("{bossbars}", () -> {
					...
					List<String> desc = CollectionUtils.asList("");
					plugin.getBossbars().forEach(
							(id, value) -> desc.add("§a" + value.getId() + " §7= §2" + StringUtils.pluralizeAmountDesc("player", value.getPlayers().size())));
					desc.sort(String::compareTo);
					return desc;
				}).send(call);
	}
