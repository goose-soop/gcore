...
public final class ParticleCompat {
    ...
    private static final ReflectionProcedureUndeciConsumer<Collection<Player>, Location, Particle, Color, Float, Integer, Integer, Float, Float, Float, Float> SEND_PARTICLE = new ReflectionProcedureUndeciConsumer<Collection<Player>, Location, Particle, Color, Float, Integer, Integer, Float, Float, Float, Float>()
            .setIf(Version.ATLEAST_1_13, (players, location, particle, color, redstoneColorScale, noteColor, count, speed, offx, offy, offz) -> {
                ...
                if (particleBukkit == null) {
                    GCore.inst().getMainLogger()
                            .warning("Couldn't display unknown particle " + particle.getData().getDataName() + " (registered as " + particle.getId() + ")");
                } else {
                    // note
                    if (particle.getData().isMusicNote()) {
                        float note = NumberUtils.isInRange(noteColor, 0, 24) ? (float) noteColor : NumberUtils.random(0f, 24f);
                        players.forEach(player -> player.spawnParticle(particleBukkit.get(), location.getX(), location.getY(), location.getZ(), count, offx,
                                offy, offz, Optional.of(note)));
                    }
                    // redstone
                    else if (particle.getData().getDataName().equals("REDSTONE")) {
                        org.bukkit.Particle.DustOptions data = new org.bukkit.Particle.DustOptions(
                                color != null ? color : Color.fromRGB(NumberUtils.random(0, 255), NumberUtils.random(0, 255), NumberUtils.random(0, 255)),
                                redstoneColorScale);
                    ...
                    // not colorable
                        ...
                        players.forEach(player -> {
                            player.spawnParticle(particleBukkit.get(), location.getX(), location.getY(), location.getZ(), useColor ? 0 : count,
                ...
                if (particleNms == null) {
                    ...
                    if (particle.getData().isMusicNote()) {
                        float note = NumberUtils.isInRange(noteColor, 0, 24) ? (float) noteColor : NumberUtils.random(0f, 24f);
                        Reflection.sendNmsPacket(players, "PacketPlayOutWorldParticles", particleNms.get(), true, (float) location.getX(),
                                (float) location.getY(), (float) location.getZ(), offx, offy, offz, note, count, emptyData);
                    }
                    // not a note
                    else {
                        boolean useColor = color != null && particle.getData().isColorable();
                        Reflection.sendNmsPacket(players, "PacketPlayOutWorldParticles", particleNms.get(), true, (float) location.getX(),
                                (float) location.getY(), (float) location.getZ(), (!useColor ? offx : color.getRed()), (!useColor ? offy : color.getGreen()),
                                (!useColor ? offz : color.getBlue()), useColor ? 1f : speed, useColor ? 0 : count, emptyData);
                    }
                }
                ...
                if (particle.getData().isMusicNote()) {
                    float note = NumberUtils.isInRange(noteColor, 0, 24) ? (float) noteColor : NumberUtils.random(0f, 24f);
                    Reflection.sendNmsPacket(players, "PacketPlayOutWorldParticles", particle.getData().getDataName(), (float) location.getX(),
                            (float) location.getY(), (float) location.getZ(), offx, offy, offz, note, count);
                }
                // not a note
                else {
                    boolean useColor = color != null && particle.getData().isColorable();
                    Reflection.sendNmsPacket(players, "PacketPlayOutWorldParticles", particle.getData().getDataName(), (float) location.getX(),
                            (float) location.getY(), (float) location.getZ(), (!useColor ? offx : color.getRed()), (!useColor ? offy : color.getGreen()),
                            (!useColor ? offz : color.getBlue()), speed, count);
                }
            });
    ...
    public static void sendParticle(Collection<Player> players, Location location, Particle particle, Color color, Float redstoneColorScale, Integer noteColor,
            int count, float speed, float offx, float offy, float offz) {
