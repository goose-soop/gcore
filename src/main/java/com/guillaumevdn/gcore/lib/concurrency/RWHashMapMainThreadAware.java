...
public class RWHashMapMainThreadAware<K, V> extends HashMap<K, V> {
	...
	public RWHashMapMainThreadAware(int initialCapacity, float loadFactor) {
	...
	public final boolean equals(Object o) { // adapted code from HashMap
		...
		return lock.read(() -> {
			...
			RWHashMapMainThreadAware<?, ?> otherRW = (RWHashMapMainThreadAware<?, ?>) ObjectUtils.castOrNull(other, RWHashMapMainThreadAware.class);
			if (otherRW != null) {
	...
	@Override
	public final RWHashMapMainThreadAware<K, V> clone() {
		return lock.read(() -> {
			RWHashMapMainThreadAware<K, V> clone = new RWHashMapMainThreadAware<>(super.size(), 1f);
			clone.putAll(this);
			return clone;
		});
	}
