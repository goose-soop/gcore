...
public class ActiveElementGUI extends ActiveGUI {
	...
	public ActiveElementGUI(ElementGUI element, Replacer replacer, Option... options) {
		super(element.getPlugin(), "instance_" + element.getId() + "_" + StringUtils.generateRandomAlphanumericString(5),
				element.getName().parse(replacer).orElse("?"), element.getSize().parse(replacer).orElse(GUIType.CHEST_6_ROW), replacer, options);
	...
	public final Collection<ItemHolder> getContents() {
		return modifiedContentsStream().sorted((a, b) -> {
			...
			boolean borderA = ObjectUtils.ifCanBeCastedDo(a, ElementGUIItemHolder.class, aa -> aa.getElement().getType().getId().contains("DYNAMIC_BORDER"))
					.orElse(false);
			boolean borderB = ObjectUtils.ifCanBeCastedDo(b, ElementGUIItemHolder.class, bb -> bb.getElement().getType().getId().contains("DYNAMIC_BORDER"))
					.orElse(false);
			if (borderA && !borderB)
				return -1;
			if (!borderA && borderB)
