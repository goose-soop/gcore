...
public class ReflectionField {
	...
	private ReflectionField(Class<?> clazz, String name) throws Throwable {
		...
		if (field == null) {
			Reflection.logAndRethrowError(new NoSuchFieldException(), "Class " + original + "\nName '" + name + "'");
		}
	}
		...
		try {
			...
			if (Modifier.isFinal(field.getModifiers())) {
				if (JavaVersion.getMajorJavaVersion() >= 16) {
					...
					ReflectionObject lookup = Reflection.invokeMethod(java.lang.invoke.MethodHandles.class, "privateLookupIn", null, Field.class,
			...
			Reflection.logAndRethrowError(throwable, "Class " + field.getDeclaringClass() + "\nName '" + field.getName() + "'" + "\nValue '" + value + "'"
