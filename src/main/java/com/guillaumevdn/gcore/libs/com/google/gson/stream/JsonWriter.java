...
import static com.guillaumevdn.gcore.libs.com.google.gson.stream.JsonScope.EMPTY_OBJECT;
...
/**
 * Writes a JSON (<a href="http://www.ietf.org/rfc/rfc7159.txt">RFC 7159</a>) encoded value to a stream, one token at a
 * time. The stream includes both literal values (strings, numbers, booleans and nulls) as well as the begin and end
 * delimiters of objects and arrays.
 *
 * <h3>Encoding JSON</h3> To encode your data as JSON, create a new {@code JsonWriter}. Each JSON document must contain
 * one top-level array or object. Call methods on the writer as you walk the structure's contents, nesting arrays and
...
public class JsonWriter implements Closeable, Flushable {
  ...
  /*
   * From RFC 7159, "All Unicode characters may be placed within the quotation marks except for the characters that must
   * be escaped: quotation mark, reverse solidus, and the control characters (U+0000 through U+001F)."
   *
   * We also escape '\u2028' and '\u2029', which JavaScript interprets as newline characters. This prevents eval() from
  ...
  /**
   * Creates a new instance that writes a JSON-encoded stream to {@code out}. For best performance, ensure {@link Writer}
   * is buffered; wrapping in {@link java.io.BufferedWriter BufferedWriter} if necessary.
   */
  ...
  /**
   * Configure this writer to relax its syntax rules. By default, this writer only emits well-formed JSON as specified by
   * <a href="http://www.ietf.org/rfc/rfc7159.txt">RFC 7159</a>. Setting the writer to lenient permits the following:
   * <ul>
  ...
  /**
   * Configure this writer to emit JSON that's safe for direct inclusion in HTML and XML documents. This escapes the HTML
   * characters {@code <}, {@code >}, {@code &} and {@code =} before writing them to the stream. Without this setting,
   * your XML/HTML encoder should replace these characters with the corresponding escape sequences.
   */
  ...
  /**
   * Inserts any necessary separators and whitespace before a literal value, inline array, or inline object. Also adjusts
   * the stack to expect either a closing bracket or another element.
   */
